...
export const ProjectDetailPage = () => {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <h2 className="text-xl font-semibold text-slate-900">{project?.title || 'Project'}</h2>
    ...
    {phases.length === 0 ? <p className="mt-2 text-sm text-slate-500">No phases available.</p> : <ul className="mt-2 space-y-2">{phases.map((phase) => <li key={phase.id} className="flex items-center justify-between rounded border border-slate-200 p-3"><span>{phase.title} ({phase.status})</span><button onClick={() => loadBudgets(phase.id)} className="text-sm font-medium text-sky-700">Load budgets</button></li>)}</ul>}
